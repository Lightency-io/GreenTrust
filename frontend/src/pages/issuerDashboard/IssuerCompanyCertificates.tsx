...
const IssuerCompanyCertificates = () => {
  const { status, razonSocial } = useParams<{ status: string, razonSocial: string }>();
        ...
        <Grid2 container spacing={3}>
          {certificates.map((certificate) => (
            <Grid2 key={certificate.id} size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
              <Card
                variant="outlined"
                onClick={() => navigate(`/issuer/${status}/${razonSocial}/certificate/${certificate.id}`)}
                sx={{
                  cursor: 'pointer',
                  height: "100%",
...
export default IssuerCompanyCertificates;
